...
local MAX_FOLLOW_CLOSE = 15
local MIN_FOLLOW = 2
local TARGET_FOLLOW = 3
local MAX_FOLLOW = 8
...
local function CryForQt(inst)
    inst.components.talker:Say(SPEECH.ARON.DISTANCIA.FAR)
    for k, v in pairs(AllPlayers) do
        if v.prefab == "jao" then
            ...
            inst:DoTaskInTime(2, function()
                inst.components.talker:Say(SPEECH.ARON.DISTANCIA.CLOSE)
            end)
            break
        end
    end
...
local SummonAron = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function SummonAron:OnStart()
